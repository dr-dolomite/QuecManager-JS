...
set quecprofiles.$profile_index.nr5g_bands='$nr5g_bands'
...
if update_profile "$profile_index" "$name" "$imei" "$apn" "$pdp_type" "$lte_bands" "$nr5g_bands" "$network_type"; then
    ...
    output_json "success" "Profile updated successfully" "{\"name\":\"$name\",\"iccid\":\"$iccid\",\"imei\":\"$imei\",\"apn\":\"$apn\",\"pdp_type\":\"$pdp_type\",\"lte_bands\":\"$lte_bands\",\"nr5g_bands\":\"$nr5g_bands\",\"network_type\":\"$network_type\"}"
else
