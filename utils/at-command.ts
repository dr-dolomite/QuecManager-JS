...
interface ATQueueResponse {
  ...
  export const atCommandSender = async (
    ...
    try {
      ...
      let url = `/cgi-bin/quecmanager/at_cmd/at_queue_client?command=${encodedCommand}`;
   ...
   */
  export const getCommandResult = async (commandId: string): Promise<ATQueueResponse> => {
    try {
      const response = await fetch(`/cgi-bin/services/at_queue_client?command_id=${commandId}`, {
        method: "GET",
        headers: {
          Accept: "application/json",
        },
   ...
   */
  export const extractATCommandOutput = (response: ATQueueResponse): string | null => {
